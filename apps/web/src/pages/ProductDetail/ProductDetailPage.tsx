...
/**
 * Renders the Specifications section for dynamic attributes.
 * Uses displayLabel from the attribute schema for each attribute label.
 * Omits optional attributes with no value (null/undefined).
 *
...
function SpecificationsSection({
  ...
  const entries = Object.entries(dynamicAttributes).filter(([, value]) => {
    // Omit attributes with no value (null/undefined/empty)
    ...
    <div className="mt-6">
      ...
      <dl className="mt-3 divide-y divide-gray-100 rounded-lg border border-gray-200 bg-white">
        {entries.map(([fieldName, value]) => {
          const displayLabel = attributeLabels[fieldName] ?? fieldName;
          ...
          return (
            <div
              key={fieldName}
              className="flex items-start justify-between gap-4 px-4 py-3 sm:grid sm:grid-cols-3"
            >
              <dt className="text-sm font-medium text-gray-600">{displayLabel}</dt>
              <dd className="text-sm text-gray-900 sm:col-span-2">{displayValue}</dd>
            </div>
          );
        })}
      </dl>
...
export function Component(): React.JSX.Element {
  ...
  // Fetch subcategories for breadcrumb
  const subcategoriesQuery = useQuery({
    queryKey: ['subcategories', product?.categoryId],
    queryFn: () => fetchSubcategories(product?.categoryId as string),
    enabled: !!product?.categoryId,
    staleTime: 1000 * 60 * 10,
  });

  const parentCategory = categoriesQuery.data?.find((c) => c.categoryId === product?.categoryId);
  const currentSubcategory = subcategoriesQuery.data?.find(
    (s) => s.categoryId === product?.subcategoryId,
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {/* Breadcrumb with category and subcategory names */}
          ...
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            {parentCategory ? (
              <>
                ...
                <li>
                  <Link
                    to={`/browse/${parentCategory.categoryId}`}
                ...
                {currentSubcategory ? (
                  <>
                    <li>
                      <Link
                        to={`/browse/${parentCategory.categoryId}/${currentSubcategory.categoryId}`}
                        className="hover:text-brand-blue-600"
                      >
            ...
            <li className="font-medium text-gray-900">{product?.name ?? 'Product Detail'}</li>
          </ol>
        </nav>
        ...
        {product && (
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div>
              ...
              {/* Specifications section — dynamic attributes with display labels */}
