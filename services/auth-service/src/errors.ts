 ...
 * Requirement 2.2
 */
export function createInvalidCredentialsError(): never {
  const error = createError(401, 'Invalid email or password', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.INVALID_CREDENTIALS;
  throw error;
}
 ...
 * Requirement 2.3, 2.4
 */
export function createAccountLockedError(): never {
  const error = createError(423, 'Account is temporarily locked. Please try again later.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.ACCOUNT_LOCKED;
  throw error;
}
 ...
 * Requirement 3.4
 */
export function createOtpExpiredError(): never {
  const error = createError(400, 'OTP has expired. Please request a new one.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.OTP_EXPIRED;
  throw error;
}
 ...
 * Requirement 3.3
 */
export function createOtpInvalidError(): never {
  const error = createError(400, 'Invalid OTP. Please try again.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.OTP_INVALID;
  throw error;
}
 ...
 * Requirement 3.3
 */
export function createOtpMaxAttemptsError(): never {
  const error = createError(400, 'Maximum OTP attempts exceeded. Please request a new OTP.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.OTP_MAX_ATTEMPTS;
  throw error;
}
 ...
 * Requirement 3.5
 */
export function createPhoneNotFoundError(): never {
  const error = createError(404, 'Phone number is not registered.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.PHONE_NOT_FOUND;
  throw error;
}
 ...
 */
export function createOtpDeliveryFailedError(): never {
  const error = createError(500, 'Unable to send OTP. Please try again later.', {
    expose: true,
  });
  (error as unknown as Record<string, unknown>)['code'] = AUTH_ERROR_CODES.OTP_DELIVERY_FAILED;
  throw error;
}
