...
function getPaymentFunctionName(): string {
  return process.env['PAYMENT_FUNCTION_NAME'] ?? '';
}

function getReturnExchangeRequestsTableName(): string {
 ...
 */
export async function checkout(buyerId: string, input: CheckoutSchemaInput): Promise<OrderRecord> {
  ...
  const productIds = cartItems.map((item) => item['productId'] as string);
  const products = await batchGetProducts(productIds);

  // Validate stock for all items
  const outOfStockItems: Array<{ productId: string; requested: number; available: number }> = [];

  for (const cartItem of cartItems) {
    const productId = cartItem['productId'] as string;
    const requestedQuantity = cartItem['quantity'] as number;
  ...
  const paymentStatus: 'Paid' | 'Pending' = paymentMethod === 'CashOnDelivery' ? 'Pending' : 'Paid';
 ...
 */
async function queryCartItems(buyerId: string): Promise<Record<string, unknown>[]> {
 ...
 */
async function getAddressSnapshot(buyerId: string, addressId: string): Promise<AddressSnapshot> {
  ...
  try {
    ...
    const line2Value = item['line2'] as string | undefined;
 ...
 */
async function invokePaymentLambda(
  orderId: string,
  amount: number,
  method: string,
  paymentDetails?: { mockCardLast4?: string; mockUpiRef?: string },
  ...
  try {
    ...
    ) as PaymentResponse & { success?: boolean; error?: { message?: string } };

    // Check if payment was unsuccessful
    if (!responsePayload.success) {
      const errorMessage =
        (responsePayload as unknown as { error?: { message?: string } }).error?.message ??
 ...
 */
async function createOrderAndDecrementStock(
    ...
    for (let i = 0; i < cartItems.length; i += batchSize) {
      ...
      const deleteRequests = batch.map((item) => ({
        DeleteRequest: {
          Key: {
            PK: `BUYER#${buyerId}`,
            SK: `PRODUCT#${item['productId'] as string}`,
          },
        },
      }));
 ...
 */
function mapItemToOrderRecord(item: Record<string, unknown>): OrderRecord {
  const record: OrderRecord = {
    orderId: item['orderId'] as string,
    buyerId: item['buyerId'] as string,
    orderTimestamp: item['orderTimestamp'] as string,
    deliveryAddressSnapshot: item['deliveryAddressSnapshot'] as AddressSnapshot,
    items: item['items'] as OrderItem[],
    paymentMethod: item['paymentMethod'] as string,
    paymentStatus: item['paymentStatus'] as OrderRecord['paymentStatus'],
    orderStatus: item['orderStatus'] as OrderRecord['orderStatus'],
    totalAmount: item['totalAmount'] as number,
  };

  if (item['transactionId']) {
    record.transactionId = item['transactionId'] as string;
  }

  const refundStatus = item['refundStatus'] as string | undefined;
  if (refundStatus) {
    record.refundStatus = refundStatus as NonNullable<OrderRecord['refundStatus']>;
  }
...
export async function getOrderHistory(
  ...
  try {
    ...
    const orders: OrderRecord[] = (result.Items ?? []).map((item) =>
      mapItemToOrderRecord(item as Record<string, unknown>),
    );

    const response: OrderHistoryResponse = { orders };
...
export async function getOrderDetail(
  ...
  try {
    ...
    const item = result.Item;
    const orderBuyerId = item['buyerId'] as string;
    ...
    return mapItemToOrderRecord(item as Record<string, unknown>);
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      throw error;
    }
...
export async function cancelOrder(
  ...
  const currentStatus = item['orderStatus'] as string;
  ...
  let refundStatus: 'Completed' | 'Pending' | undefined;
 ...
 */
async function invokeRefundLambda(
  orderId: string,
  amount: number,
  method: string,
): Promise<PaymentResponse> {
  ...
  const responsePayload = JSON.parse(
    new TextDecoder().decode(response.Payload),
  ) as PaymentResponse & { success?: boolean };
...
export async function createReturnExchangeRequest(
  orderId: string,
  buyerId: string,
  input: ReturnExchangeRequestSchemaInput,
  ...
  let orderItem: Record<string, unknown>;
  try {
    const getCommand = new GetCommand({
      TableName: ordersTableName,
      Key: {
    ...
    orderItem = result.Item as Record<string, unknown>;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      throw error;
    }
  ...
  // Assert ownership
  const orderBuyerId = orderItem['buyerId'] as string;
  ...
  const orderStatus = orderItem['orderStatus'] as string;
  ...
  const orderItems = orderItem['items'] as Array<{ productId: string }> | undefined;
  ...
  let sellerPolicy: Record<string, unknown> | undefined;
  try {
    ...
    sellerPolicy = productResult.Item['sellerPolicy'] as Record<string, unknown> | undefined;
  } catch (error) {
    if (error && typeof error === 'object' && 'statusCode' in error) {
      throw error;
    }
  ...
  const returnWindowDays = sellerPolicy['returnWindowDays'] as number;
  ...
  const orderTimestamp = orderItem['orderTimestamp'] as string;
  ...
  const policyVersionAtRequest = sellerPolicy['policyVersion'] as string;

  const returnExchangeRecord: ReturnExchangeRequest = {
    requestId,
    orderId,
    buyerId,
    type: input.type,
 ...
 * Requirements: 13.4
 */
export async function getReturnExchangeRequestDetail(
  requestId: string,
  requestingBuyerId: string,
): Promise<ReturnExchangeRequest> {
  const returnExchangeTableName = getReturnExchangeRequestsTableName();
  ...
  try {
    ...
    const item = result.Item;
    const requestBuyerId = item['buyerId'] as string;
    ...
    const record: ReturnExchangeRequest = {
      requestId: item['requestId'] as string,
      orderId: item['orderId'] as string,
      buyerId: item['buyerId'] as string,
      type: item['type'] as ReturnExchangeRequest['type'],
      status: item['status'] as ReturnExchangeRequest['status'],
      policyVersionAtRequest: item['policyVersionAtRequest'] as string,
      createdAt: item['createdAt'] as string,
    };

    // Include sellerNotes if present
    const sellerNotes = item['sellerNotes'] as string | undefined;
    if (sellerNotes) {
