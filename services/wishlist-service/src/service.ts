...
import type { WishlistItemEnriched, WishlistResponse } from '@blipzo/shared';
...
function getWishlistsTableName(): string {
 ...
 */
export async function getWishlist(buyerId: string): Promise<WishlistResponse> {
  try {
...
async function enrichWishlistItems(
  ...
  const enrichedItems: WishlistItemEnriched[] = wishlistItems.map((wishlistItem) => {
    const sk = wishlistItem['SK'] as string;
    const productId = sk.replace('PRODUCT#', '');
    const addedAt = wishlistItem['addedAt'] as string;
    ...
    const isDeleted = product['isDeleted'] as boolean | undefined;
 ...
 */
export async function addToWishlist(buyerId: string, productId: string): Promise<WishlistResponse> {
    ...
    if (error instanceof Error && error.name === 'TransactionCanceledException') {
      ...
      if (message.includes('ConditionalCheckFailed')) {
        ...
        const cancellationReasons = (
          error as unknown as { CancellationReasons?: Array<{ Code?: string }> }
        )['CancellationReasons'];
