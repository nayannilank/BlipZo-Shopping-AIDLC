...
import { createProduct, updateProduct, deleteProduct, listSellerProducts, setSellerPolicy, } from './service.js';
import { validateCreateProductInput, validateUpdateProductInput, validateSellerPolicyInput, extractSellerId, extractProductId, extractPaginationParams, } from './validators.js';
/**
 ...
 */
const rawCreateProductHandler = async (event) => {
 ...
 */
const rawUpdateProductHandler = async (event) => {
    const sellerId = extractSellerId(event);
 ...
 */
const rawDeleteProductHandler = async (event) => {
    const sellerId = extractSellerId(event);
    const productId = extractProductId(event);
 ...
 * Requirements: 5.3
 */
const rawListSellerProductsHandler = async (event) => {
    const sellerId = extractSellerId(event);
 ...
 */
const rawSetSellerPolicyHandler = async (event) => {
    const sellerId = extractSellerId(event);
    const productId = extractProductId(event);
